# frozen_string_literal: true

RSpec.describe ElementComponent::Components::Row do
  describe "default row" do
    subject { ElementComponent::Components::Row.new }

    it "has the correct element name" do
      expect(subject.element).to eq("div")
    end

    it "has row class" do
      expect(subject.attributes[:class]).to include("row")
    end

    it "renders basic row" do
      subject.add_content("content")
      expect(subject.render).to eq('<div class="row">content</div>')
    end
  end

  describe "with cols" do
    it "renders row-cols-2" do
      row = ElementComponent::Components::Row.new(cols: 2)
      expect(row.render).to eq('<div class="row row-cols-2"></div>')
    end

    it "renders row-cols-3" do
      row = ElementComponent::Components::Row.new(cols: 3)
      expect(row.render).to eq('<div class="row row-cols-3"></div>')
    end
  end

  describe "with cols as hash" do
    it "renders row-cols-2 row-cols-md-3" do
      row = ElementComponent::Components::Row.new(cols: { default: 2, md: 3 })
      expect(row.render).to eq('<div class="row row-cols-2 row-cols-md-3"></div>')
    end

    it "renders row-cols-2 row-cols-md-3 row-cols-lg-4" do
      row = ElementComponent::Components::Row.new(cols: { default: 2, md: 3, lg: 4 })
      expect(row.render).to eq('<div class="row row-cols-2 row-cols-md-3 row-cols-lg-4"></div>')
    end

    it "renders row-cols-md-3 row-cols-lg-4" do
      row = ElementComponent::Components::Row.new(cols: { md: 3, lg: 4 })
      expect(row.render).to eq('<div class="row row-cols-md-3 row-cols-lg-4"></div>')
    end
  end

  describe "with gutters" do
    it "renders with gutter" do
      row = ElementComponent::Components::Row.new(gutter: 3)
      expect(row.render).to eq('<div class="row g-3"></div>')
    end

    it "renders with gutter_x" do
      row = ElementComponent::Components::Row.new(gutter_x: 2)
      expect(row.render).to eq('<div class="row gx-2"></div>')
    end

    it "renders with gutter_y" do
      row = ElementComponent::Components::Row.new(gutter_y: 4)
      expect(row.render).to eq('<div class="row gy-4"></div>')
    end

    it "renders with both gutter_x and gutter_y" do
      row = ElementComponent::Components::Row.new(gutter_x: 2, gutter_y: 3)
      expect(row.render).to eq('<div class="row gx-2 gy-3"></div>')
    end
  end

  describe "with gutters as hash" do
    it "renders with gutter hash" do
      row = ElementComponent::Components::Row.new(gutter: { default: 2, md: 3 })
      expect(row.render).to eq('<div class="row g-2 g-md-3"></div>')
    end

    it "renders with gutter_x hash" do
      row = ElementComponent::Components::Row.new(gutter_x: { default: 2, md: 3 })
      expect(row.render).to eq('<div class="row gx-2 gx-md-3"></div>')
    end

    it "renders with gutter_y hash" do
      row = ElementComponent::Components::Row.new(gutter_y: { default: 2, md: 3 })
      expect(row.render).to eq('<div class="row gy-2 gy-md-3"></div>')
    end
  end

  describe "with block content" do
    subject do
      ElementComponent::Components::Row.new do |r|
        r << "content"
      end
    end

    it "renders with content" do
      expect(subject.render).to eq('<div class="row">content</div>')
    end
  end

  describe "with custom class" do
    subject { ElementComponent::Components::Row.new(class: "custom") }

    it "preserves custom class" do
      expect(subject.attributes[:class]).to include("custom")
      expect(subject.attributes[:class]).to include("row")
    end
  end

  describe "dynamic methods" do
    describe "#cols" do
      it "adds cols after initialization" do
        row = ElementComponent::Components::Row.new
        row.cols(2)
        expect(row.render).to eq('<div class="row row-cols-2"></div>')
      end

      it "adds cols with hash" do
        row = ElementComponent::Components::Row.new
        row.cols(md: 3, lg: 4)
        expect(row.render).to eq('<div class="row row-cols-md-3 row-cols-lg-4"></div>')
      end

      it "adds cols to existing row" do
        row = ElementComponent::Components::Row.new(cols: 2)
        row.cols(md: 3)
        expect(row.render).to eq('<div class="row row-cols-2 row-cols-md-3"></div>')
      end

      it "returns self for chaining" do
        row = ElementComponent::Components::Row.new
        expect(row.cols(2)).to eq(row)
      end
    end

    describe "#gutter" do
      it "adds gutter after initialization" do
        row = ElementComponent::Components::Row.new
        row.gutter(3)
        expect(row.render).to eq('<div class="row g-3"></div>')
      end

      it "adds gutter with hash" do
        row = ElementComponent::Components::Row.new
        row.gutter(default: 2, md: 3)
        expect(row.render).to eq('<div class="row g-2 g-md-3"></div>')
      end

      it "returns self for chaining" do
        row = ElementComponent::Components::Row.new
        expect(row.gutter(3)).to eq(row)
      end
    end

    describe "#gutter_x" do
      it "adds gutter_x after initialization" do
        row = ElementComponent::Components::Row.new
        row.gutter_x(2)
        expect(row.render).to eq('<div class="row gx-2"></div>')
      end

      it "returns self for chaining" do
        row = ElementComponent::Components::Row.new
        expect(row.gutter_x(2)).to eq(row)
      end
    end

    describe "#gutter_y" do
      it "adds gutter_y after initialization" do
        row = ElementComponent::Components::Row.new
        row.gutter_y(4)
        expect(row.render).to eq('<div class="row gy-4"></div>')
      end

      it "returns self for chaining" do
        row = ElementComponent::Components::Row.new
        expect(row.gutter_y(4)).to eq(row)
      end
    end

    describe "chaining" do
      it "chains multiple methods" do
        row = ElementComponent::Components::Row.new
        row.cols(2).gutter_x(2).gutter_y(3)
        expect(row.render).to eq('<div class="row row-cols-2 gx-2 gy-3"></div>')
      end
    end
  end
end
